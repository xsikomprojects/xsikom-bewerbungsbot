...
def tipp():
    """Gibt einen zufälligen Bewerbungstipp zurück."""
    return random.choice([
        ...
        "Bewerbung als PDF senden.",
        "Ansprechpartner namentlich ansprechen.",
    ])
...
def bp(u, p):
    """Benutzer prüfen / Login."""
    cn = sqlite3.connect(DB)
    c  = cn.cursor()
    c.execute(
        "SELECT id, benutzername, vorname, nachname, "
        "rolle, premium FROM benutzer "
...
def ba(u, p, e, v, n, k="privat"):
    """Benutzer anlegen / Registrierung."""
    try:
        cn = sqlite3.connect(DB)
        c  = cn.cursor()
        c.execute(
            "INSERT INTO benutzer "
            "(benutzername, passwort, email, vorname, "
            "nachname, kunde_typ, erstellt) "
            "VALUES (?,?,?,?,?,?,?)",
            (u, hp(p), e, v, n, k,
...
def pa(uid):
    ...
    c  = cn.cursor()
    c.execute(
        "UPDATE benutzer SET premium=1 WHERE id=?", (uid,)
    )
    cn.commit()
    cn.close()
...
def bz(uid):
    """Bewerbungsanzahl diesen Monat."""
    cn = sqlite3.connect(DB)
    c  = cn.cursor()
    c.execute(
        "SELECT COUNT(*) FROM bewerbungen "
...
def ds(file, uid, kat):
    ...
    c.execute(
        "INSERT INTO uploads "
        "(user_id, dateiname, typ, kategorie, pfad, upload_datum) "
        "VALUES (?,?,?,?,?,?)",
        (uid, nn, ext, kat, pfad,
         datetime.now().isoformat())
    )
...
def get_theme(uid):
    """Gibt Theme des Users zurück."""
    try:
        cn = sqlite3.connect(DB)
        c  = cn.cursor()
        c.execute(
            "SELECT theme FROM benutzer WHERE id=?", (uid,)
        )
        r = c.fetchone()
        cn.close()
...
def alle_tags_user(uid):
    """Gibt alle einzigartigen Tags zurück."""
    try:
        cn = sqlite3.connect(DB)
        c  = cn.cursor()
        c.execute(
...
def chart_daten_laden(uid):
    """Lädt Chart-Daten für Dashboard."""
    try:
        cn = sqlite3.connect(DB)
        c  = cn.cursor()
...
def erfolgs_statistik(uid):
    """Berechnet Erfolgs-Statistiken."""
    try:
        cn = sqlite3.connect(DB)
        c  = cn.cursor()
        ...
        c.execute(
            ...
            "('antwort','interview','zusage','absage')",
            (uid,)
        )
        mit_antwort = c.fetchone()[0] or 0

        c.execute(
            "SELECT COUNT(*) FROM bewerbungen "
            "WHERE user_id=? AND status='zusage'",
            (uid,)
        )
        zusagen = c.fetchone()[0] or 0

        c.execute(
            "SELECT COUNT(*) FROM bewerbungen "
            "WHERE user_id=? AND status='absage'",
            (uid,)
        )
        absagen = c.fetchone()[0] or 0
        ...
        interviews = c.fetchone()[0] or 0

        c.execute(
            "SELECT strftime('%Y-%m', datum) as monat, "
            "COUNT(*) FROM bewerbungen "
            "WHERE user_id=? GROUP BY monat "
            "ORDER BY monat DESC LIMIT 6",
            (uid,)
        )
        pro_monat = c.fetchall()

        c.execute(
            "SELECT typ, COUNT(*) FROM bewerbungen "
            "WHERE user_id=? GROUP BY typ",
            (uid,)
        )
        pro_typ = c.fetchall()
        cn.close()

        antwortrate    = round((mit_antwort / gesamt * 100), 1) if gesamt > 0 else 0.0
        erfolgsrate    = round((zusagen     / gesamt * 100), 1) if gesamt > 0 else 0.0
        interview_rate = round((interviews  / gesamt * 100), 1) if gesamt > 0 else 0.0

        return {
            "gesamt":         gesamt,
            "mit_antwort":    mit_antwort,
            "zusagen":        zusagen,
            "absagen":        absagen,
...
def teilen_links(url, text):
    """Erstellt Teilen-Links für verschiedene Plattformen."""
    enc_url  = urllib.parse.quote(url)
    enc_text = urllib.parse.quote(text)
    return {
        "whatsapp": f"https://wa.me/?text={enc_text}%20{enc_url}",
        "telegram": f"https://t.me/share/url?url={enc_url}&text={enc_text}",
        "twitter":  f"https://twitter.com/intent/tweet?text={enc_text}&url={enc_url}",
        "linkedin": f"https://www.linkedin.com/sharing/share-offsite/?url={enc_url}",
        "email":    f"mailto:?subject={enc_text}&body={enc_url}",
        "copy":     url,
    }
...
def dbi():
    ...
    c.execute(
        "CREATE TABLE IF NOT EXISTS bewerbung_tags ("
        "id INTEGER PRIMARY KEY AUTOINCREMENT,"
        "user_id INTEGER,"
...
def aa():
    ...
    c  = cn.cursor()
    c.execute(
        "SELECT id FROM benutzer WHERE benutzername='admin'"
    )
    if not c.fetchone():
        c.execute(
            "INSERT INTO benutzer "
            "(benutzername, passwort, email, vorname, "
            "nachname, rolle, premium, erstellt) "
            "VALUES (?,?,?,?,?,?,?,?)",
            (
                "admin",
                hp("XsiKOM2026!"),
...
H = (
    '<!DOCTYPE html><html lang="de" id="html-root"><head>'
    '<meta charset="UTF-8">'
    ...
    'wght@300;400;500;600;700&family=Space+Grotesk:wght@500;700'
    ...
    ':root{--bg:#0A0E1A;--cd:rgba(20,28,48,0.6);'
    '--bd:rgba(255,255,255,0.08);--cy:#00D9FF;--pu:#8B5CF6;'
    '--gn:#10F4B1;--yl:#FFD93D;--rd:#FF4757;'
    '--t1:#FFF;--t2:#A0AEC0;--t3:#6B7280}'
    "body{font-family:'Poppins',sans-serif;background:var(--bg);"
    'color:var(--t1);min-height:100vh;transition:background 0.3s,color 0.3s}'
    "body::before{content:'';position:fixed;top:0;left:0;right:0;bottom:0;"
    'background:radial-gradient(circle at 20% 20%,'
    'rgba(0,217,255,0.15) 0%,transparent 50%),'
    'radial-gradient(circle at 80% 80%,'
    'rgba(139,92,246,0.15) 0%,transparent 50%);z-index:-1}'
    '.ct{max-width:1200px;margin:0 auto;padding:20px}'
    '.hd{background:rgba(10,14,26,0.8);backdrop-filter:blur(20px);'
    'padding:20px 0;border-bottom:1px solid var(--bd);'
    'position:sticky;top:0;z-index:100}'
    '.hi{display:flex;justify-content:space-between;align-items:center}'
    ".lg{font-family:'Space Grotesk',sans-serif;font-size:32px;"
    'font-weight:700;background:linear-gradient(135deg,var(--cy),var(--pu));'
    '-webkit-background-clip:text;background-clip:text;'
    '-webkit-text-fill-color:transparent}'
    '.st{color:var(--t2);font-size:13px}'
    '.tm{background:rgba(0,217,255,0.1);border:1px solid var(--cy);'
    'color:var(--t1);padding:8px 14px;border-radius:12px;cursor:pointer;'
    'font-size:18px;transition:all 0.3s;margin-left:10px}'
    '.tm:hover{background:rgba(0,217,255,0.2);transform:translateY(-2px)}'
    '.nv{background:rgba(19,24,41,0.5);padding:12px 0;'
    'border-bottom:1px solid var(--bd);overflow-x:auto;white-space:nowrap}'
    '.ni{max-width:1200px;margin:0 auto;padding:0 20px;'
    'display:flex;gap:5px;align-items:center}'
    '.nv a{color:var(--t2);text-decoration:none;padding:10px 18px;'
    'border-radius:12px;font-size:13px;transition:all 0.3s;font-weight:500}'
    '.nv a:hover{color:var(--t1);background:rgba(0,217,255,0.15);'
    'transform:translateY(-2px)}'
    '.cd{background:var(--cd);backdrop-filter:blur(20px);'
    'border-radius:20px;padding:30px;margin:20px 0;'
    'border:1px solid var(--bd);transition:all 0.4s}'
    '.cd:hover{transform:translateY(-5px);'
    'border-color:rgba(0,217,255,0.3)}'
    '.bt{display:inline-flex;align-items:center;justify-content:center;'
    ...
    'input,textarea,select{background:rgba(10,14,26,0.6);'
    'border:1px solid var(--bd);color:var(--t1);'
    'padding:14px 18px;border-radius:12px;width:100%;'
    'margin-bottom:12px;font-size:14px;transition:all 0.3s}'
    'input:focus,textarea:focus,select:focus{'
    'outline:none;border-color:var(--cy);'
    'box-shadow:0 0 0 4px rgba(0,217,255,0.1)}'
    "h1{font-family:'Space Grotesk',sans-serif;font-size:36px;"
    'font-weight:700;background:linear-gradient(135deg,var(--cy),var(--pu));'
    '-webkit-background-clip:text;background-clip:text;'
    ...
    '.gr{display:grid;grid-template-columns:'
    'repeat(auto-fit,minmax(240px,1fr));gap:20px}'
    '.sc{background:linear-gradient(135deg,rgba(20,28,48,0.8),'
    'rgba(30,38,58,0.6));border:1px solid var(--bd);'
    'border-radius:20px;padding:30px;text-align:center;'
    'transition:all 0.4s;cursor:pointer}'
    '.sc:hover{transform:translateY(-8px);border-color:var(--cy)}'
    '.si{font-size:48px;margin-bottom:12px}'
    ".sv{font-family:'Space Grotesk',sans-serif;font-size:32px;"
    'font-weight:700;background:linear-gradient(135deg,var(--cy),var(--pu));'
    '-webkit-background-clip:text;background-clip:text;'
    '-webkit-text-fill-color:transparent}'
    '.sl{color:var(--t2);font-size:13px;margin-top:4px}'
    '.bg{background:linear-gradient(135deg,var(--yl),#EC4899);'
    'color:#0A0E1A;padding:6px 14px;border-radius:20px;'
    'font-size:11px;font-weight:700;display:inline-block}'
    '.al{padding:16px 20px;border-radius:12px;margin:16px 0;'
    'border:1px solid}'
    '.ao{background:rgba(16,244,177,0.1);'
    'border-color:rgba(16,244,177,0.3);color:var(--gn)}'
    '.ae{background:rgba(255,71,87,0.1);'
    'border-color:rgba(255,71,87,0.3);color:var(--rd)}'
    '.aw{background:rgba(255,217,61,0.1);'
    'border-color:rgba(255,217,61,0.3);color:var(--yl)}'
    '.ai{background:rgba(0,217,255,0.1);'
    'border-color:rgba(0,217,255,0.3);color:var(--cy)}'
    '.ui{background:rgba(10,14,26,0.6);padding:16px;'
    'border-radius:12px;margin:10px 0;display:flex;'
    'justify-content:space-between;align-items:center;'
    'border:1px solid var(--bd)}'
    '.tg{display:inline-flex;align-items:center;gap:6px;'
    'padding:4px 12px;border-radius:20px;font-size:12px;'
    'font-weight:600;margin:3px;border:1px solid rgba(255,255,255,0.2)}'
    '.tg-cy{background:rgba(0,217,255,0.15);color:var(--cy);'
    'border-color:var(--cy)}'
    '.tg-gn{background:rgba(16,244,177,0.15);color:var(--gn);'
    'border-color:var(--gn)}'
    '.tg-yl{background:rgba(255,217,61,0.15);color:var(--yl);'
    'border-color:var(--yl)}'
    '.tg-rd{background:rgba(255,71,87,0.15);color:var(--rd);'
    'border-color:var(--rd)}'
    '.tg-pu{background:rgba(139,92,246,0.15);color:var(--pu);'
    'border-color:var(--pu)}'
    '.tg-t2{background:rgba(160,174,192,0.15);color:var(--t2);'
    'border-color:var(--t2)}'
    '.tg-t3{background:rgba(107,114,128,0.15);color:var(--t3);'
    'border-color:var(--t3)}'
    '.ch{position:relative;height:300px;margin:20px 0}'
    '.ft{background:rgba(10,14,26,0.9);padding:40px 20px 30px;'
    'text-align:center;color:var(--t3);margin-top:60px;'
    'border-top:1px solid var(--bd)}'
    '.ft a{color:var(--t2);margin:0 12px}'
    ".fb{margin-top:16px;font-family:'Space Grotesk',sans-serif;"
    'font-weight:600;color:var(--cy)}'
    '#cb{display:none;position:fixed;bottom:20px;left:20px;right:20px;'
    'max-width:1160px;margin:0 auto;background:rgba(20,28,48,0.95);'
    'color:white;padding:20px 25px;z-index:9999;'
    'border-radius:16px;border:1px solid var(--cy)}'
    '.lt{background:var(--cd);padding:30px;border-radius:20px;'
    'margin:20px 0;line-height:1.8;border:1px solid var(--bd)}'
    '.lt h3{color:var(--cy);margin-top:24px}'
    '@media(max-width:768px){'
    'h1{font-size:28px}.lg{font-size:24px}.ch{height:200px}}'
    '</style></head><body>'
    ...
    '<div>🍪 Cookies. '
    '<a href="/datenschutz" style="color:var(--cy)">Mehr</a>'
    '</div>'
    '<button onclick="cookieAccept()" class="bt b2">✓</button>'
    '</div></div>'

    # Header
    '<div class="hd"><div class="ct hi"><div>'
    '<div style="display:flex;align-items:center;gap:12px">'
    '<img src="/static/logo.png" alt="XsiKOM" style="height:45px" '
    "onerror=\"this.style.display='none'\">"
    '<div class="lg">XsiKOM</div></div>'
    '<div class="st">'
    '{{ user.vorname if user else "KI Bewerbungs-Assistent" }}'
    '</div></div>'
    '<button class="tm" id="theme-btn" '
    ...
    '© 2026 XsiKOM DIGITAL Projects<br>'
    '<a href="mailto:xsikom_digital@xsikom.de" '
    'style="color:var(--t3)">xsikom_digital@xsikom.de</a>'
    '</div>'
    '</div>'
