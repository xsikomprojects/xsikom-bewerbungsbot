...
def _footer():
    """Gibt Standard-Footer für Legal-Seiten zurück."""
    return (
        '<p style="margin-top:30px;padding:16px;'
        'background:rgba(10,14,26,0.5);'
        'border-radius:12px;font-size:12px">'
        '<strong>Stand:</strong> Juni 2026<br>'
        '<strong>© 2026 XsiKOM DIGITAL Projects - Komi Tevi</strong>'
        '</p>'
    )


def _kontakt_block():
    """Gibt Standard-Kontaktblock zurück."""
    return (
        ...
        'Deutschland<br>'
        f'E-Mail: <a href="mailto:{CE}">{CE}</a><br>'
        'Web: https://xsikom.de</p>'
    )


def _render(c):
    """Rendert Legal-Seite mit user-Parameter."""
...
def register_legal_routes(app):
    ...
    def impressum():
        c = (
            ...
            '<p>Als Diensteanbieter sind wir gemaess § 7 Abs. 1 TMG '
            'fuer eigene Inhalte verantwortlich. Nach §§ 8 bis 10 TMG '
            'sind wir nicht verpflichtet, uebermittelte Informationen '
            'zu ueberwachen.</p>'

            '<h3>Haftung fuer Links</h3>'
            '<p>Unser Angebot enthaelt Links zu externen Webseiten. '
            'Fuer verlinkte Seiten ist der jeweilige Anbieter '
            'verantwortlich.</p>'

            '<h3>Urheberrecht</h3>'
            '<p>Inhalte und Werke unterliegen dem deutschen '
            'Urheberrecht.</p>'
    ...
    def datenschutz():
        c = (
            ...
            '<li>Nutzungsdaten (Login-Zeiten, anonymisiert)</li>'
            '</ul>'

            '<h3>3. Zwecke der Verarbeitung</h3>'
            '<p>Bereitstellung des Bewerbungs-Service '
            ...
            '</ul>'
            f'<p>Anfragen: <a href="mailto:{CE}">{CE}</a></p>'

            '<h3>5. KI-Verarbeitung (Aaliyah, AVINU, XSI)</h3>'
            '<p>Bei Nutzung der KI werden Eingaben an Groq Inc. (USA) '
            'uebermittelt. EU-US Data Privacy Framework. Keine '
            'personenbezogenen Daten werden dauerhaft bei Groq '
            'gespeichert.</p>'
            ...
            '<li><strong>CSRF-Token:</strong> Sicherheit</li>'
            '</ul>'
    ...
    def agb():
        c = (
            ...
            '<li>Alle Jobportale + International</li>'
            '</ul>'
            ...
            '<p><strong>Wichtig:</strong> Keine Garantie fuer Erfolg '
            'von Bewerbungen. KI-Inhalte koennen Fehler enthalten.</p>'
    ...
    def widerruf():
        c = (
            ...
            '<p><strong>Sie haben das Recht, binnen 14 Tagen '
            'ohne Angabe von Gruenden diesen Vertrag zu widerrufen.'
            '</strong></p>'
            ...
            '<p>Kein gesetzliches Widerrufsrecht. '
    ...
    def haftung():
        c = (
            ...
            '<li>KI-generierte Inhalte koennen '
            '<strong>fehlerhaft</strong> sein</li>'
            '<li>Pruefen Sie alle Inhalte vor Verwendung</li>'
            '<li>Keine Garantie fuer Erfolg von Bewerbungen</li>'
            '<li>KI-Empfehlungen ersetzen keine professionelle '
            'Beratung</li>'
            ...
            '<p>Haftung nur bei Vorsatz und grober '
            'Fahrlaessigkeit.</p>'
            ...
            '<h3>Kontakt</h3>'
            f'<p>E-Mail: <a href="mailto:{CE}">{CE}</a></p>'
